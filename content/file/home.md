---
title: clacktronics.co.uk
style: page
tagline: Hardware for people who dont bother to read the datasheet twice.
---
# Clacktronics

Welcome to CLACKTRONICS a project and company site for all things made by Ben Barwise. Based in a shed in North-East London, Epping forest, I build and design electronic equipment and solutions for artists and musicians. I have 20+ years experience as a technologist in music and art and am a PCB design specialist. I am currently a full-time technician at UCL in East London.

---

## What is clackOS?

A corny mock operating system that has a number of useful apps and content for music, art and just about anything else I can think of. Most apps are either derived by me or open source projects, you will find credits at the bottom of each app. The whole site runs client side and managed using an open [Github repository](https://github.com/clacktronics/clacktronics_website). If you see any issues please let me know in the bug report found in the File menu. It will work ok on mobile, but its best viewed on desktop.

## Euroclack

![Euroclack synthesizer on a desk with a keyboard and the book](https://clacktronics.co.uk/content/file/euroclack/byom/images/BYOM_built_closeup.jpg)

The main electronic output I create at the moment are my open source modular synthesizer projects. Please look at the calendar top right for upcoming events where I will be showing new in development work, click below to have a look at all the relavent projects. You will also find distributors there where you can buy my kits and modules.

[Browse EuroClack](window:file/euroclack.md)

## The Blog

![643_edp_wasp_deluxe_main643](assets/old_assets/643_edp_wasp_deluxe_main643.jpg)

Some of this information is ported over from my last site, which in turn was imported from the Wordpress site before that, I hope to update the blog more frequently than I have previously, it has been 10 years of no movement on this website at publishing.

[Read the blog](window:file/blog.md)


## The rest

![Press'n'peel](https://clacktronics.co.uk/content/file/misc/pressnpeel/images/643_press-n-peel-blue-pcb.jpg)

Below are articles and bits that dont fit in, reverse engineering, thoughts, articles etc..

[Misc](window:file/misc.md) 
[Archive](window:file/archive/archive.md)
[Links](window:file/links.md)




